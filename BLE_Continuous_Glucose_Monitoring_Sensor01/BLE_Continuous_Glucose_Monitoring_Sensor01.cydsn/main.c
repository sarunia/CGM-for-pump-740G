...
CYBLE_GAP_SMP_KEY_DIST_T keyDist;

void printKeyDistribution(const CYBLE_GAP_SMP_KEY_DIST_T* keyDist)
{
    DBG_PRINTF("Long Term Key (LTK):\r\n");
    for (int i = 0; i < CYBLE_GAP_SMP_LTK_SIZE; i++) {
        DBG_PRINTF("%02X ", keyDist->ltkInfo[i]);
    }
    DBG_PRINTF("\r\n");

    DBG_PRINTF("Encrypted Diversifier and Random Number (MID):\r\n");
    for (int i = 0; i < CYBLE_GAP_SMP_MID_INFO_SIZE; i++) {
        DBG_PRINTF("%02X ", keyDist->midInfo[i]);
    }
    DBG_PRINTF("\r\n");

    DBG_PRINTF("Identity Resolving Key (IRK):\r\n");
    for (int i = 0; i < CYBLE_GAP_SMP_IRK_SIZE; i++) {
        DBG_PRINTF("%02X ", keyDist->irkInfo[i]);
    }
    DBG_PRINTF("\r\n");

    DBG_PRINTF("Public Device/Static Random Address (ID Address):\r\n");
    for (int i = 0; i < CYBLE_GAP_SMP_IDADDR_DATA_SIZE; i++) {
        DBG_PRINTF("%02X ", keyDist->idAddrInfo[i]);
    }
    DBG_PRINTF("\r\n");

    DBG_PRINTF("Connection Signature Resolving Key (CSRK):\r\n");
    for (int i = 0; i < CYBLE_GAP_SMP_CSRK_SIZE; i++) {
        DBG_PRINTF("%02X ", keyDist->csrkInfo[i]);
    }
    DBG_PRINTF("\r\n");
}
...
// Example structure for characteristic info
typedef struct {
    CYBLE_GATT_DB_ATTR_HANDLE_T handle;
    CYBLE_UUID16 uuid; // assuming 16-bit UUIDs
} CharacteristicInfo;

// Example array of characteristic info
CharacteristicInfo characteristicInfo[] = {
    {0x000E, 0x2A00},
    {0x0010, 0x2A01},
    // Add all other handles and their corresponding UUIDs here
};

// Declare the function prototype before use
CYBLE_UUID16 GetUUIDFromHandle(CYBLE_GATT_DB_ATTR_HANDLE_T handle);

CYBLE_UUID16 GetUUIDFromHandle(CYBLE_GATT_DB_ATTR_HANDLE_T handle) {
    for (int i = 0; i < sizeof(characteristicInfo) / sizeof(characteristicInfo[0]); i++) {
        if (characteristicInfo[i].handle == handle) {
            return characteristicInfo[i].uuid;
        }
    }
    return 0; // Return 0 if UUID not found
}
...
{   
    ...
    DBG_PRINTF("Device connected, bdHandle: %x\r\n", cyBle_connHandle.bdHandle);
    DBG_PRINTF("Security level: %x\r\n", cyBle_authInfo.security);
    DBG_PRINTF("Initiating pairing process\r\n");
    ...
    if((cyBle_authInfo.security & CYBLE_GAP_SEC_LEVEL_MASK) > CYBLE_GAP_SEC_LEVEL_1)
    {
        ...
        {
            DBG_PRINTF(" CyBle_GapAuthReq API Error: %d\r\n", apiResult);
            PrintApiResult();
        }
        else
        {
            DBG_PRINTF("CyBle_GapAuthReq API called successfully\r\n");
        }
    }
    else
    {
        authState = AUTHENTICATION_BONDING_COMPLETE;
        DBG_PRINTF("Authentication bonding complete\r\n");
    }

    // Dodatkowe informacje diagnostyczne o stanie diod LED
    DBG_PRINTF("LED_OFF: %d\r\n", LED_OFF);
    DBG_PRINTF("LED_ON: %d\r\n", LED_ON);
    DBG_PRINTF("Disconnect LED state before write: %d\r\n", LED_OFF);
    DBG_PRINTF("Advertising LED state before write: %d\r\n", LED_OFF);
    DBG_PRINTF("Connected LED state before write: %d\r\n", LED_ON);

    Disconnect_LED_Write(LED_OFF);
    Advertising_LED_Write(LED_OFF);
    Connected_LED_Write(LED_ON);

    // Sprawdzenie aktualnego stanu diod LED po aktualizacji
    DBG_PRINTF("Disconnect LED state after write: %d\r\n", LED_OFF);
    DBG_PRINTF("Advertising LED state after write: %d\r\n", LED_OFF);
    DBG_PRINTF("Connected LED state after write: %d\r\n", LED_ON);

    DBG_PRINTF("LEDs updated: Disconnect OFF, Advertising OFF, Connected ON\r\n");
    
    // Drukowanie kluczy po ustawieniu na urządzeniach BLE
    //printKeyDistribution(&keyDist);
        ...
        ***********************************************************/
        ...
        case CYBLE_EVT_GATTS_READ_CHAR_VAL_ACCESS_REQ:
            ...
            * event parameter. */
            ...
            {
                CYBLE_GATTS_CHAR_VAL_READ_REQ_T *readReq = (CYBLE_GATTS_CHAR_VAL_READ_REQ_T *)eventParam;
                CYBLE_GATT_DB_ATTR_HANDLE_T attrHandle = readReq->attrHandle;
                CYBLE_UUID16 uuid = GetUUIDFromHandle(attrHandle);

                DBG_PRINTF("CYBLE_EVT_GATTS_READ_CHAR_VAL_ACCESS_REQ: handle: %x \r\n", attrHandle);
                if (uuid != 0) { // assuming 16-bit UUIDs
                    DBG_PRINTF("UUID: %x \r\n", uuid);
                } else {
                    DBG_PRINTF("UUID not found for handle: %x \r\n", attrHandle);
                }
            }
            break;
...
int main()
{
    ...
    CYBLE_GAP_SMP_KEY_DIST_T keyDist = {
        .ltkInfo = {0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D, 0x0E, 0x0F},
        .midInfo = {0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07},
        .irkInfo = {0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D, 0x0E, 0x0F},
        .idAddrInfo = {0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06},
        .csrkInfo = {0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D, 0x0E, 0x0F}
    };
