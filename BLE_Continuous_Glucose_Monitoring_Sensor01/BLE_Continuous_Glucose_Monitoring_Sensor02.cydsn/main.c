...
// Deklaracje funkcji
//void PrintUUID(CYBLE_UUID_T uuid, uint8 uuidFormat);
//void PrintCharacteristicInfo(const CYBLE_DISC_CHAR_INFO_T *charInfo);
    ...
    switch(event)
    {
        ...
        ***********************************************************/
        ...
        case CYBLE_EVT_GATTS_XCNHG_MTU_REQ:
            ...
            //CYBLE_DISC_CHAR_INFO_T *discCharInfo = (CYBLE_DISC_CHAR_INFO_T *)eventParam;
                //PrintCharacteristicInfo(discCharInfo);
            }
            break;
...
// Funkcja konwertująca UUID do formatu string
/*void PrintUUID(CYBLE_UUID_T uuid, uint8 uuidFormat) {
    char uuidStr[37]; // 36 characters for 128-bit UUID + null terminator
    if (uuidFormat == 0x01) { // 16-bit UUID
        DBG_PRINTF("UUID format: 16-bit\r\n");
        sprintf(uuidStr, "UUID: 0x%04X", uuid.uuid16);
    } else if (uuidFormat == 0x02) { // 128-bit UUID
        DBG_PRINTF("UUID format: 128-bit\r\n");
        sprintf(uuidStr, "UUID: %08X-%04X-%04X-%04X-%08X%04X",
                uuid.uuid128.value[12], uuid.uuid128.value[10], uuid.uuid128.value[8],
                uuid.uuid128.value[6], uuid.uuid128.value[0], uuid.uuid128.value[4]);
    } else {
        DBG_PRINTF("Unknown UUID format: 0x%02X\r\n", uuidFormat);
        return;
    }
    DBG_PRINTF("UUID string: %s\r\n", uuidStr);
}

// Funkcja drukująca szczegóły struktury CYBLE_DISC_CHAR_INFO_T
void PrintCharacteristicInfo(const CYBLE_DISC_CHAR_INFO_T *charInfo) {
    char buffer[100];
    sprintf(buffer, "Characteristic Declaration Handle: 0x%04X\r\n", charInfo->charDeclHandle);
    DBG_PRINTF("%s", buffer);

    sprintf(buffer, "Properties: 0x%02X\r\n", charInfo->properties);
    DBG_PRINTF("%s", buffer);

    sprintf(buffer, "Value Handle: 0x%04X\r\n", charInfo->valueHandle);
    DBG_PRINTF("%s", buffer);

    DBG_PRINTF("UUID Format: 0x%02X\r\n", charInfo->uuidFormat); // Debugowanie formatu UUID
    DBG_PRINTF("Printing UUID...\r\n");
    PrintUUID(charInfo->uuid, charInfo->uuidFormat);
    DBG_PRINTF("Finished printing UUID.\r\n");
}*/
